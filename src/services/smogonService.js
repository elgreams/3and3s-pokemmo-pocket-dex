...
async function loadCachedPayload(type, tierId) {
  try {
    const raw = await getItem(getStorageKey(type, tierId));
    if (!raw) return null;
    const parsed = JSON.parse(raw);
    if (!parsed || typeof parsed !== 'object') return null;
    if (parsed.timestamp && parsed.timestamp + CACHE_TTL_MS < Date.now()) {
      await removeItem(getStorageKey(type, tierId));
      return null;
    }
...
async function storePayload(type, tierId, data) {
  try {
    const payload = JSON.stringify({ timestamp: Date.now(), data });
    await setItem(getStorageKey(type, tierId), payload);
  } catch (err) {
...
async function ensureTierSets(tierId) {
  ...
  let payload = await loadCachedPayload('sets', tierId);
  if (!payload) {
    payload = await fetchJson(meta.setsUrl);
    await storePayload('sets', tierId, payload);
  }
...
async function ensureTierAnalyses(tierId) {
  ...
  const meta = tierById.get(tierId);
  if (!meta) throw new Error(`Unknown Smogon tier: ${tierId}`);
  let payload = await loadCachedPayload('analyses', tierId);
  if (!payload) {
    payload = await fetchJson(meta.analysesUrl);
    await storePayload('analyses', tierId, payload);
  }
