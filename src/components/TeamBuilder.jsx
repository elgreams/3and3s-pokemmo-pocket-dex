...
export default function TeamBuilder({ onViewMon }) {
  const [shinyGlobal, setShinyGlobal] = React.useState(false);
  React.useEffect(() => {
    let mounted = true;
  ...
  React.useEffect(() => {
    let mounted = true;
    getJson('teamBuilderSavedTeams', {}).then((saved) => {
      if (!mounted) return;
      if (saved && typeof saved === 'object') setSavedTeams(saved);
    });
  ...
  const handleSave = () => {
    ...
    setSavedTeams(prev => {
      ...
      void setJson('teamBuilderSavedTeams', next);
      return next;
    });
    setSaveName('');
