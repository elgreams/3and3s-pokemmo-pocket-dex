...
export default function ColorPickerButton({ renderTrigger = true }){
  ...
  const overlayStyle = {
    ...
    justifyContent:'center', zIndex:10000, padding:0
  };
  const modalStyle = {
    background:'var(--surface)', color:'var(--text)', padding:20,
    width:'100%', maxWidth:1100, height:'100%', maxHeight:'100%', minHeight:0,
    overflowY:'auto', borderRadius:'var(--radius-lg)', boxShadow:'var(--shadow-2)'
  };
  ...
  const onSave = () => {
    ...
    setMethodColors(m);
    setRarityColors(r);
    void setJson('methodColors', m);
    void setJson('rarityColors', r);
    setOpen(false);
  };
  const onDefault = () => {
