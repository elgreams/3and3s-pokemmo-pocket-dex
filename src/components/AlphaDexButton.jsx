...
import { getBoolean, getJson, setJson } from '../services/storage';
...
function buildDefaultEventOpen(){
  const initial = {};
...
export default function AlphaDexButton(){
  ...
  const [eventOpen, setEventOpen] = useState(buildDefaultEventOpen);
  useEffect(() => {
    let mounted = true;
    getJson('alphaEventOpenMap', null).then((saved) => {
      if (!mounted || !saved || typeof saved !== 'object') return;
      setEventOpen({ ...buildDefaultEventOpen(), ...saved });
    });
    return () => { mounted = false; };
  }, []);
  useEffect(() => {
    void setJson('alphaEventOpenMap', eventOpen);
  }, [eventOpen]);
